		...
		init : function(){ 
			...
			if(this.feeds['youtube_arr']) {
				for(var i = 0; i < this.feeds['youtube_arr'].length; i++) {
					this.feeds['youtube_url'][i] = 'https://gdata.youtube.com/feeds/base/users/'+$.trim(this.feeds['youtube_arr'][i])+'/uploads';
				}
			...
			if(this.feeds['youtube_search_arr']) {
				for(var i = 0; i < this.feeds['youtube_search_arr'].length; i++) {
					this.feeds['youtube_search_url'][i] = 'https://gdata.youtube.com/feeds/api/videos?alt=atom&racy=include&vq='+$.trim(this.feeds['youtube_search_arr'][i]);
				}
			...
			for (x = num; x < this.entry.length; x++)
			{
				...
				if(this.entry[x].link.indexOf('twitter.com') !== -1) {
					...
					if(typeof title != "undefined") {
						...
						title = title.replace("nolink", "https");
					}
